...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Bolt Optimization: Pre-allocate String and use lookup table to avoid dynamic allocations
    // in this hot path, replacing the previous `.map(|byte| format!("{byte:02x}")).collect()`
    let mut res = String::with_capacity(64);
    const HEX_CHARS: &[u8; 16] = b"0123456789abcdef";
    for byte in hash {
        res.push(HEX_CHARS[(byte >> 4) as usize] as char);
        res.push(HEX_CHARS[(byte & 0x0f) as usize] as char);
    }
    res
}
