...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Performance optimization: Avoid dynamic memory allocations (`format!`) in the FUSE hot path.
    // Use pre-allocated Strings and a custom bitwise lookup for hex encoding.
