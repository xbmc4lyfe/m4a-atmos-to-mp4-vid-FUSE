...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // ⚡ Bolt Optimization: Use a pre-allocated string with bitwise lookup
    // instead of iterator mapping with format! to avoid 32 dynamic memory
    // allocations in this hot path called on FUSE getattr requests.
