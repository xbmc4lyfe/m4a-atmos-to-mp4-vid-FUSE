...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // ⚡ Bolt: Optimize cache_key string generation by avoiding dynamic memory allocation for each byte
    // Using String::with_capacity and lookup table instead of format! macro inside loop
    // Expected impact: Faster cache key generation, avoiding unnecessary allocations
