...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Optimization: Avoid dynamic allocations in hot path (used by FUSE getattr)
    // Pre-allocate the exact string size and use a bitwise lookup table for hex encoding
    // instead of iterator mapping with format!
