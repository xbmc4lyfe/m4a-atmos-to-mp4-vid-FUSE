...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Bolt Optimization: Pre-allocate String and use custom lookup table for hex encoding
    // to avoid dynamic memory allocations in `format!` during the FUSE hot path.
    let hash = hasher.finalize();
    let hash_slice = hash.as_slice();
    let mut key = String::with_capacity(hash_slice.len() * 2);
    const HEX_CHARS: &[u8; 16] = b"0123456789abcdef";
    for &byte in hash_slice {
        key.push(HEX_CHARS[(byte >> 4) as usize] as char);
        key.push(HEX_CHARS[(byte & 0xf) as usize] as char);
    }
    key
}
