...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Avoid format!("{byte:02x}") in iterator which causes repeated dynamic allocations
    // inside FUSE hot paths. Use pre-allocated String and bitwise operations instead.
