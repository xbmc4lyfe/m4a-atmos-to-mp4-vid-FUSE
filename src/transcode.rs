...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // OPTIMIZATION: Use pre-allocated String and bitwise operations to avoid dynamic
    // allocation overhead of `format!("{byte:02x}")` inside map/collect.
    // This is a hot path for FUSE cache lookups.
