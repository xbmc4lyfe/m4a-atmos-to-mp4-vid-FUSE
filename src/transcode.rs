...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Optimize: Hex encode the hash directly to a pre-allocated string
    // avoiding `format!` which allocates a new String for every single byte.
    // This is called per-file when FUSE accesses size via cache.
