...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Performance optimization: Using pre-allocated String and lookup table
    // instead of iterator mapping with format! avoids repeated dynamic
    // memory allocations on FUSE hot paths.
