...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // PERFORMANCE OPTIMIZATION: Avoid 32 dynamic memory allocations from `format!("{byte:02x}")`
    // by using a pre-allocated String and a custom bitwise lookup table for hex encoding.
