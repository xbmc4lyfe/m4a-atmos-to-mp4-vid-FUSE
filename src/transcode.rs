...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Optimize hot loop: use a pre-allocated string and lookup table
    // instead of format! to prevent repeated dynamic allocations
    let hash = hasher.finalize();
    let mut result = String::with_capacity(hash.as_slice().len() * 2);
