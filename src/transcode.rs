...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Performance Optimization: FUSE filesystem accessed paths are hot paths.
    // Avoid formatting with `format!` and dynamic mapping for encoding.
    // Use a pre-allocated string with a lookup table.
