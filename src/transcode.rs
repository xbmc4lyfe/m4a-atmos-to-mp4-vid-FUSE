...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Performance optimization: Avoid dynamic memory allocation in hot path.
    // By pre-allocating a String with exact capacity (64 chars for Sha256)
    // and using bitwise lookup table for hex encoding, we avoid allocating
    // multiple Strings using `format!` and iterator map which are slow.
