...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // FUSE hot path optimization: pre-allocate exact capacity for 32 hex bytes (64 chars).
    // Avoids 32 dynamic memory allocations from `format!` per calculation.
    let hash = hasher.finalize();
    let mut result = String::with_capacity(64);
    for byte in hash {
        // We use bitwise logic and predefined chars instead of `format!("{byte:02x}")`
