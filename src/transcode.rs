...
pub fn cache_key(path: &Path, size: u64, mtime: SystemTime) -> String {
    ...
    // Optimized hex string generation:
    // Avoids format! and iterator mapping which cause repeated dynamic allocations.
    // Uses pre-allocated String and bitwise lookup table for high performance on FUSE hot paths.
